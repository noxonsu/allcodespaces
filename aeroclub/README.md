...
*   Backend настроен с CORS для разрешения запросов с `http://localhost:3003` (и других распространенных портов разработки, или `*` для открытого доступа во время разработки).
...
3.  **Запустите сервер разработки:**
    ...
    npm start
    ```
    Приложение будет доступно по адресу `http://localhost:3008` (или другому порту, если 3000 занят).

### Как открыть в Telegram
1.  **Запустите приложение локально**, как описано выше.
2.  **Используйте ngrok** или аналогичный сервис, чтобы создать публичный HTTPS-тоннель к вашему локальному серверу (`localhost:3008`).
    ```bash
    ngrok http 3008
    ```
...
-   **Панель администратора**: `http://localhost:3003`
-   **Клиентское приложение**: `http://localhost:3008`

## Задание для тестировщика

### Общие цели:
*   Проверить работоспособность основного функционала всех частей приложения (Admin Panel, Client App, Backend).
*   Убедиться в корректности взаимодействия между frontend и backend.
*   Выявить ошибки, несоответствия и неудобства в пользовательском интерфейсе.

### Backend API (проверка через Admin Panel или инструменты типа Postman/Insomnia)

*   **Аутентификация и Авторизация:**
    *   Вход администратора (`/api/v1/auth/token`).
    *   Доступ к защищенным эндпоинтам с валидным токеном.
    *   Отказ в доступе без токена или с невалидным токеном.
    *   Проверка прав доступа для эндпоинта создания пользователя (`/api/v1/users/`) - только администратор.
    *   Первоначальная настройка администратора (`/api/v1/auth/setup-admin`).
*   **Управление пользователями:**
    *   Создание нового пользователя.
    *   Получение списка пользователей.
    *   Получение данных текущего пользователя (`/api/v1/users/me/`).
    *   (Если реализовано) Редактирование и удаление пользователей.
*   **Управление пунктами меню (Menu Items):**
...
*   **Управление заказами (Orders):**
    ...
    *   Создание нового заказа (проверить логику, особенно если заказ создается из админ-панели).
*   **Управление локациями (Locations/Scaling):**
    *   Создание новой локации.
    *   Просмотр списка локаций.
    *   Редактирование существующей локации.
    *   Удаление локации.
    *   Генерация QR-кодов для локаций.
*   **Обработка ошибок:**
    *   Проверка корректности кодов ответа и сообщений об ошибках при неверных запросах или серверных проблемах.
...
*   **Страница входа:**
    ...
    *   Перенаправление на страницу администратора после входа.
*   **Страница администратора (основные разделы):**
    *   **Пользователи:**
        *   Отображение списка пользователей.
        *   Создание нового пользователя через форму.
        *   (Если реализовано) Редактирование и удаление пользователей.
        *   Поиск и фильтрация (если есть).
    *   **Пункты меню:**
        *   Отображение списка пунктов меню.
        *   Загрузка/добавление нового пункта меню с изображением и данными.
        *   Редактирование существующего пункта меню.
        *   Удаление пункта меню.
    *   **Заказы:**
        ...
        *   Изменение статуса заказа.
    *   **Масштабирование/Локации:**
        *   Отображение списка локаций.
        *   Создание новой локации.
        *   (Если реализовано) Редактирование и удаление локаций.
        *   Генерация и отображение QR-кода для локации.
*   **Общее для Admin Panel:**
    *   Корректное отображение данных, полученных с бэкенда.
    *   Работа кнопок, форм, переключателей и других элементов UI.
    *   Обработка состояний загрузки и ошибок при взаимодействии с API.
    *   Функциональность выхода из системы.
    *   Адаптивность интерфейса (если предусмотрена).

### Клиентское приложение (`client-app` - Telegram Mini App)

*   **Запуск и отображение:**
    *   Корректный запуск приложения внутри Telegram.
    *   Отображение основного интерфейса (`ClientAppPage`).
*   **Основной функционал (в зависимости от реализации):**
    *   Просмотр меню/каталога.
    *   Формирование заказа.
    *   Отправка заказа на бэкенд.
    *   Отображение статуса заказа.
    *   Взаимодействие с элементами управления, специфичными для Telegram Mini App (кнопки, данные пользователя Telegram и т.д.).
*   **Взаимодействие с Backend:**
    *   Корректная отправка запросов на бэкенд.
    *   Правильное отображение данных, полученных с бэкенда.
    *   Обработка ошибок API.
*   **Пользовательский опыт:**
    *   Интуитивность навигации.
    *   Скорость работы приложения.
    *   Общее удобство использования в среде Telegram.

### Интеграционное тестирование:
*   Создать заказ в `client-app` -> проверить его появление и корректность данных в `admin-app` и через API `backend`.
*   Изменить данные (например, пункт меню) в `admin-app` -> проверить, что изменения отразились в `client-app` (после обновления) и через API `backend`.
*   Создать пользователя в `admin-app` -> попробовать войти этим пользователем (если предусмотрен такой сценарий).

### Нефункциональное тестирование (базовые проверки):
*   **Удобство использования (Usability):** Общая оценка интуитивности и простоты интерфейсов.
*   **Совместимость:** (Если актуально) Проверка работы Admin Panel в разных браузерах. Проверка Client App на разных устройствах и версиях Telegram.
