...
# Для этого split_image.py должен быть в той же директории или в PYTHONPATH
SPLIT_IMAGE_AVAILABLE = False
split_image_intellectually = None

try:
    from .split_image import split_image_intellectually
    SPLIT_IMAGE_AVAILABLE = True
    print("Модуль split_image успешно импортирован из относительного пути")
except ImportError:
    try:
        # Если запускается как главный скрипт, а не как модуль
        from split_image import split_image_intellectually
        SPLIT_IMAGE_AVAILABLE = True
        print("Модуль split_image успешно импортирован из абсолютного пути")
    except ImportError as e:
        print(f"ВНИМАНИЕ: Модуль split_image не найден: {e}")
        print("Функция разделения изображений будет недоступна")
        SPLIT_IMAGE_AVAILABLE = False
...
async def fetch_all_data_and_analyze_figma(figma_url: str) -> Dict[str, Any]:
    ...
    page_id_to_name_map = {}
    intermediate_analyses = []  # Добавляем список для промежуточных анализов
            ...
            if node_ids_to_fetch_images:
                ...
                if node_images_paths:
                    ...
                    for node_id, local_image_path in node_images_paths.items():
                        ...
                        # Убираем кеширование - всегда выполняем новый анализ изображений
                        ...
                        if actual_page_width > COLUMN_WIDTH_THRESHOLD:
                            ...
                            if 'да' in is_multi_column_response.lower():
                                ...
                                try:
                                    ...
                                    if not SPLIT_IMAGE_AVAILABLE:
                                        print("ВНИМАНИЕ: Функция split_image_intellectually недоступна")
                                        page_image_analysis_response_parts.append("ВНИМАНИЕ: Модуль разделения изображений недоступен.\n")
                                        # Fallback - анализируем широкое изображение как есть
                                        wide_image_notice = (
                                            f"ВНИМАНИЕ: Это изображение страницы '{page_name}' (ID: {node_id}) ОЧЕНЬ ШИРОКОЕ "
                                            f"(ширина холста: {actual_page_width}px). Модуль разделения изображений недоступен. "
                                            f"LLM определил {num_expected_columns} колонок на этом изображении. "
                                            "Пожалуйста, проанализируй все видимые экраны/макеты на этом широком изображении по отдельности.\n\n"
                                        )
                                        page_image_analysis_response_parts.append(await analyze_figma_data_with_llm(wide_image_notice + current_page_analysis_prompt, local_image_path, 6000))
                                    else:
                                        ...
                                        print(f"Путь для сохранения колонок: {columns_output_dir_for_image}")
                                        ...
                                        print(f"Вызываем split_image_intellectually с параметрами:")
                                        print(f"  - image_src: {local_image_path}")
                                        print(f"  - expected_columns: {num_expected_columns}")
                                        
                                        split_columns_meta = None
                                        try:
                                            ...
                                            print(f"Результат split_image_intellectually: {split_columns_meta}")
                                        except NameError as name_error:
                                            print(f"Ошибка в функции split_image_intellectually - неопределенная переменная: {name_error}")
                                            print("Возможно, в коде split_image.py есть опечатка или неинициализированная переменная")
                                            split_columns_meta = None
                                        except Exception as split_error:
                                            print(f"Общая ошибка при вызове split_image_intellectually: {split_error}")
                                            split_columns_meta = None
                                        ...
                                        if split_columns_meta and len(split_columns_meta) > 0:
                                            ...
                                            # Проверяем, действительно ли файлы были созданы
                                            print(f"Проверяем наличие файлов колонок в директории: {columns_output_dir_for_image}")
                                            
                                            # Проверяем разные возможные паттерны имен файлов
                                            possible_patterns = ['column_*.png', 'column*.png', 'col_*.png', 'part_*.png', '*.png']
                                            created_files = []
                                            
                                            for pattern in possible_patterns:
                                                files = list(columns_output_dir_for_image.glob(pattern))
                                                if files:
                                                    created_files = sorted(files)
                                                    print(f"Найдены файлы по паттерну '{pattern}': {[f.name for f in created_files]}")
                                                    break
                                            
                                            if not created_files:
                                                print(f"Файлы колонок не найдены. Содержимое директории {columns_output_dir_for_image}:")
                                                if columns_output_dir_for_image.exists():
                                                    all_files = list(columns_output_dir_for_image.iterdir())
                                                    for file in all_files:
                                                        print(f"  - {file.name}")
                                                    if all_files:
                                                        # Используем найденные файлы, если они есть
                                                        created_files = sorted([f for f in all_files if f.suffix.lower() in ['.png', '.jpg', '.jpeg']])
                                                else:
                                                    print(f"  Директория не существует!")
                                            
                                            print(f"Найдено файлов колонок для анализа: {len(created_files)}")
                                            
                                            if created_files:
                                                for i, column_file_path in enumerate(created_files):
                                                    print(f"Анализирую колонку {i+1}/{len(created_files)} (файл: {column_file_path})...")
                                                    
                                                    # Промпт для анализа ОДНОЙ КОЛОНКИ
                                                    column_analysis_prompt = (
                                                        f"Это изображение одной из {len(created_files)} колонок (колонка {i+1}) "
                                                        f"оригинальной страницы '{page_name}'. \n\n"
                                                        f"{current_page_analysis_prompt}"
                                                    )
                                                    
                                                    try:
                                                        col_analysis_response = await analyze_figma_data_with_llm(column_analysis_prompt, str(column_file_path), 4000)
                                                        page_image_analysis_response_parts.append(f"\nАнализ колонки {i+1} ({column_file_path.name}):\n{col_analysis_response}\n")
                                                        print(f"Анализ колонки {i+1} завершен успешно.")
                                                    except Exception as analysis_error:
                                                        print(f"Ошибка при анализе колонки {i+1}: {analysis_error}")
                                                        page_image_analysis_response_parts.append(f"\nАнализ колонки {i+1}: Ошибка при анализе - {analysis_error}\n")
                                            else:
                                                print("Файлы колонок не найдены для анализа.")
                                                page_image_analysis_response_parts.append("\nФайлы колонок не найдены для анализа.\n")
                                                
                                                # Fallback: анализируем целое изображение с подробным описанием проблемы
                                                fallback_notice = (
                                                    f"ВНИМАНИЕ: Это изображение страницы '{page_name}' (ID: {node_id}) ОЧЕНЬ ШИРОКОЕ "
                                                    f"(ширина холста: {actual_page_width}px). Разделение на {num_expected_columns} колонок не удалось - "
                                                    "файлы не были созданы. Пожалуйста, проанализируй все видимые экраны/макеты на этом широком изображении, "
                                                    f"учитывая что их должно быть {num_expected_columns}. Опиши каждый экран отдельно.\n\n"
                                                )
                                                page_image_analysis_response_parts.append(await analyze_figma_data_with_llm(fallback_notice + current_page_analysis_prompt, local_image_path, 6000))
                                        else:
                                            print("Не удалось разделить изображение на колонки, анализируем целиком.")
                                            if split_columns_meta is None:
                                                print("ОШИБКА: Функция разделения изображений вернула None (возможно, ошибка в коде split_image.py).") # Keep console log
                                            else:
                                                print(f"Функция разделения вернула пустой результат: {split_columns_meta}") # Keep console log
                                            
                                            # Fallback - анализируем широкое изображение с указанием количества колонок
                                            wide_image_notice = (
                                                f"ВНИМАНИЕ: Это изображение страницы '{page_name}' (ID: {node_id}) ОЧЕНЬ ШИРОКОЕ "
                                                f"(ширина холста: {actual_page_width}px). Разделение на колонки не удалось из-за ошибки в функции разделения. "
                                                f"LLM определил {num_expected_columns} колонок на этом изображении. "
                                                "Пожалуйста, проанализируй все видимые экраны/макеты на этом широком изображении, "
                                                f"считая что их должно быть {num_expected_columns}. Опиши каждый экран отдельно.\n\n"
                                            )
                                            page_image_analysis_response_parts.append(await analyze_figma_data_with_llm(wide_image_notice + current_page_analysis_prompt, local_image_path, 6000))
                        ...
                        # Добавляем промежуточный анализ в список для отправки пользователю
                        intermediate_analyses.append({
                            'page_name': page_name,
                            'node_id': node_id,
                            'analysis': final_page_analysis,
...
"""
    ...
    # Убираем кеширование итогового анализа - всегда выполняем новый
    ...
    return {
        ...
        "intermediateAnalyses": intermediate_analyses  # Добавляем промежуточные анализы
    }
...
if __name__ == '__main__':
    ...
    print("Анализатор Figma файлов запущен.")
    print("Убедитесь, что переменные окружения FIGMA_API_KEY и OPENAI_API_KEY установлены.")
    print(f"Загружен IMAGE_ANALYSE_PROMPT_TEMPLATE: {'Да' if IMAGE_ANALYSE_PROMPT_TEMPLATE else 'Нет'}")
    print(f"Загружен FINAL_ANALYSE_PROMPT_TEMPLATE: {'Да' if FINAL_ANALYSE_PROMPT_TEMPLATE else 'Нет'}")
    print()

    try:
        # Запрашиваем URL у пользователя
        figma_url = get_figma_url_from_user()
        print(f"\nНачинаю анализ файла: {figma_url}")
        print("Это может занять несколько минут...")
        print()
        
        # Запускаем анализ
        result = asyncio.run(fetch_all_data_and_analyze_figma(figma_url))
        
        print("\n" + "="*50)
        print("АНАЛИЗ ЗАВЕРШЕН УСПЕШНО!")
        print("="*50)
        print(f"Данные сохранены в: {result['dataPath']}")
        print(f"Итоговый анализ: {result['summaryFilePath']}")
        
    except KeyboardInterrupt:
        print("\nАнализ прерван пользователем.")
    except Exception as e:
        print(f"\nОшибка при выполнении анализа: {e}")
        print("Проверьте правильность ссылки и наличие API ключей.")

    # ...existing commented test code...
