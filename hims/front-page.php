...
.lazy-load-wrapper {
    ...
    transition: opacity 0.01s; /* Minimal transition just in case */
}
.lazy-load-wrapper.is-visible {
    opacity: 1;
}
/* Ensure wrappers don't add unwanted space if their content has margins */
.lazy-load-wrapper > *:first-child {
    margin-top: 0;
}
.lazy-load-wrapper > *:last-child {
    margin-bottom: 0;
}
...
document.addEventListener('DOMContentLoaded', function() {
    // Lazy Load Sections Visibility Toggle Logic (Keep this)
    const lazySections = document.querySelectorAll('.lazy-load-wrapper');
    ...
    if ("IntersectionObserver" in window) {
        let observer = new IntersectionObserver((entries, observer) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    // NOTE: JS Component initialization is now handled in index.js
                    observer.unobserve(entry.target); // Make visible only once
                }
            });
        ...
        // Consider triggering JS initializations here for fallback if needed
        // e.g., if (typeof initSwiperWhenVisible === 'function') { /* call init functions */ }
    }

    // Image Modal Logic for Written Reviews (Keep this - it's just event listeners)
    const modal = document.getElementById('imageModal');
    const modalImg = document.getElementById('modalImage');
    const closeBtn = document.querySelector('#imageModal .close-modal'); // Use specific selector
    const images = document.querySelectorAll('.review-image');

    if (modal && modalImg && closeBtn) { // Add checks
        images.forEach(img => {
    ...
    // Certificate Modal Logic is now handled within index.js's DOMContentLoaded listener

    // Common ESC key closing for both modals (Keep this)
