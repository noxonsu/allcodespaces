...
class CampaignChannelQs(QuerySet):
    ...
    def campaigns_subqs(self):
        from core.models import Campaign
        campaigns_channels_qs = self.filter(campaign_id=OuterRef('pk'), )
        return Campaign.objects.filter(id=Subquery(campaigns_channels_qs.values('campaign_id')[:1]))

    def anon_campaign_statistics(self):
        return self.campaigns_subqs.annotate(
            all_budget_fact=Sum(F('campaigns_channel__cpm') * F('campaigns_channel__impressions_fact')),
            all_budget_plan=Sum(F('campaigns_channel__cpm') * F('campaigns_channel__impressions_plan')),
        ).values('id', 'name', 'budget', 'all_budget_fact', 'all_budget_plan', 'status')

    def update_campaign_activity(self):
        from core.models import Campaign
        to_update = self.anon_campaign_statistics().filter(all_budget_fact__gte=F('budget'), status=Campaign.Statuses.ACTIVE)
        return Campaign.objects.filter(id__in=to_update.values('id')).update(status=Campaign.Statuses.PAUSED)
    ...
    def campaign_channels_total_budgets(self):
       return self.aggregate(
           total_budgets=Sum('impressions_plan') / Decimal(1000) * Sum('cpm'))['total_budgets']
