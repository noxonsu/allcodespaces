...
class QuerySetExporter(ExporterContract):
    def _prepare_data(self):
        ...
        _rows = []
        if self._data:
            _rows = [list(i.values()) for i in ExporterSerializer(instance=self._data, many=True, context={'user': self.for_user}).data]
        self._export_data.extend(_rows)

    def _prepare_cols(self):
        cols = ExporterSerializer(instance=self._data.first(), context={"user": self.for_user}).get_cols_names()
