...
class CampaignChannelFilterSet(FilterSet):
    ...
    def filter_is_message_published(self, queryset, name, value):
        ...
        return queryset.filter(
            ~Q(publish_status=CampaignChannel.PublishStatusChoices.PUBLISHED)
        )

    def filter_words(
        self, queryset, name, words_seperated: str
    ) -> QuerySet[CampaignChannel]:
        ...
        words_set: set[str] = set(
            map(
                lambda x: x.lower(),
                filter(
                    lambda x: x is not None and len(x) > 0, words_seperated.split(",")
                ),
            )
        )
        ...
        for campaign_channel in queryset.all():
            bad_set: set[str] = set(
                map(lambda x: x.lower().strip(), campaign_channel.campaign.black_list)
            )
            white_set: set[str] = set(
                map(lambda x: x.lower().strip(), campaign_channel.campaign.white_list)
            )
            print(f"{bad_set=}")
            print(f"{white_set=}")
            ...
            if (
                ...
                and (not white_set and not bad_set)
                or campaign_channel.is_approved
                and not words_set
            ):
