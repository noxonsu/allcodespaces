...
def on_commit(apps, schema):
    ...
    CampaignChannel.objects.update(
        publish_status=Case(
            When(
                Q(is_message_published=True, is_approved=True)
                | Q(message_publish_date__isnull=False),
                then=Value("published"),
            ),
            When(
                Q(is_approved=True)
                & Q(
                    Q(is_message_published=False) | Q(message_publish_date__isnull=True)
                ),
                then=Value("confirmed"),
            ),
            When(
                Q(is_approved=False, is_message_published=False), then=Value("planned")
            ),
